...
namespace NN.Geometry
{
    ...
    public struct Line : IEquatable<Line>, IEpsilonComparable<Line>
    {
        ...
        public Line(Rhino.Geometry.Line f)
        {
            ...
            this.m_from = new Point3d(f.From);
        }

        public bool CopyFrom(Rhino.Geometry.Line from)
        {
            if (from == null)
                return false;

            this.m_to = new Point3d(from.To);
            this.m_from = new Point3d(from.From);

            return true;
        }


        public bool CopyTo(Rhino.Geometry.Line to)
        {
            to.To = m_to.RhinoObject();
            to.From = m_from.RhinoObject();
            return true;
        }

        public Rhino.Geometry.Line RhinoObject()
        {
            return new Rhino.Geometry.Line(m_from.RhinoObject(), m_to.RhinoObject());
        }
#endif
