...
namespace NN.Geometry
{
    ...
    public struct Plane : IEquatable<Plane>, IEpsilonComparable<Plane>
    {
        ...
        public Plane(Rhino.Geometry.Plane f)
        {
            ...
            this.m_xaxis = new Vector3d(f.XAxis.X, f.XAxis.Y, f.XAxis.Z);
            this.m_yaxis = new Vector3d(f.YAxis.X, f.YAxis.Y, f.YAxis.Z);
            this.m_zaxis = new Vector3d(f.ZAxis.X, f.ZAxis.Y, f.ZAxis.Z);
            this.IsValid = f.IsValid;

            //CopyFrom(f);
        }

        public bool CopyFrom(Rhino.Geometry.Plane from)
        {

            if (from == null)
                return false;

            this.m_origin = new Point3d(from.Origin);
            this.m_xaxis = new Vector3d(from.XAxis);
            this.m_yaxis = new Vector3d(from.YAxis);
            this.m_zaxis = new Vector3d(from.ZAxis);
        ...
        public bool CopyTo(Rhino.Geometry.Plane to)
        {

            to.Origin = new Rhino.Geometry.Point3d(Origin.X, Origin.Y, Origin.Z);
            to.XAxis = new Rhino.Geometry.Vector3d(m_xaxis.X, m_xaxis.Y, m_xaxis.Z);
            to.YAxis = new Rhino.Geometry.Vector3d(m_yaxis.X, m_yaxis.Y, m_yaxis.Z);
            to.ZAxis = new Rhino.Geometry.Vector3d(m_zaxis.X, m_zaxis.Y, m_zaxis.Z);

            return true;
        }

        public Rhino.Geometry.Plane RhinoObject()
        {
            return new Rhino.Geometry.Plane(Origin.RhinoObject(), m_xaxis.RhinoObject(), m_yaxis.RhinoObject());
        }
#endif
      ...
      : this()
        ...
        public bool Rotate(double sinAngle, double cosAngle, Vector3d axis, Point3d centerOfRotation)
        {
            ...
            {
                Transform rot = NN.Geometry.Transform.Rotation(sinAngle, cosAngle, axis, Point3d.Origin);
                XAxis = rot * XAxis;
                YAxis = rot * YAxis;
                ZAxis = rot * ZAxis;
                return true;
            }
            Transform rot2 = NN.Geometry.Transform.Rotation(sinAngle, cosAngle, axis, centerOfRotation);
